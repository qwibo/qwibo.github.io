...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Hero language cycle (home stat) ---
  document.querySelectorAll('[data-lang-cycle]').forEach((container) => {
    let langs;
    try {
      langs = JSON.parse(container.dataset.langs || '[]');
    } catch (e) {
      return;
    }
    if (!langs.length) return;

    const codeEl = container.querySelector('.lang-cycle__code');
    const labelEl = container.querySelector('.lang-cycle__label');
    if (!codeEl || !labelEl) return;

    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reducedMotion) {
      codeEl.textContent = langs.map((l) => l.code).join(' · ');
      labelEl.textContent = container.dataset.staticLabel || langs.map((l) => l.label).join(' · ');
      return;
    }
    ...
    function swap(nextIndex) {
      codeEl.classList.add('lang-cycle--out');
      labelEl.classList.add('lang-cycle--out');

      setTimeout(() => {
        codeEl.textContent = langs[nextIndex].code;
        labelEl.textContent = langs[nextIndex].label;
        codeEl.classList.remove('lang-cycle--out');
        labelEl.classList.remove('lang-cycle--out');
        codeEl.classList.add('lang-cycle--in');
        labelEl.classList.add('lang-cycle--in');

        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            codeEl.classList.remove('lang-cycle--in');
            labelEl.classList.remove('lang-cycle--in');
          });
        });
      }, fadeMs);
    }
